 ...
 */
template <typename T>
void squared_edt_1d_multi_seg( T *segids, float *d, const long int n, const long long int stride, const float anistropy,
 ...
 */
void squared_edt_1d_parabolic( float *f, const long int n, const long int stride, const float anisotropy,
                               const bool black_border_left, const bool black_border_right )
{
  if ( n == 0 )
  {
    return;
...
// about 5% faster
void squared_edt_1d_parabolic( float *f, const int n, const long int stride, const float anisotropy )
{
  if ( n == 0 )
  {
    return;
...
void _squared_edt_1d_parabolic( float *f, const int n, const long int stride, const float anisotropy,
                                const bool black_border_left, const bool black_border_right )
{
  if ( black_border_left && black_border_right )
  {
 ...
 */
template <typename T>
void squared_edt_1d_parabolic_multi_seg( T *segids, float *f, const int n, const long int stride,
                                         const float anisotropy, const bool black_border = false )
{
  T working_segid = segids[ 0 ];
  T segid;
  long int last = 0;
  ...
  for ( int i = 1; i < n; i++ )
  {
    ...
    if ( segid != working_segid )
    {
      if ( working_segid != 0 )
      {
        _squared_edt_1d_parabolic( f + last * stride, i - last, stride, anisotropy, ( black_border || last > 0 ),
                                   true );
      }
      working_segid = segid;
      last          = i;
  ...
  {
    _squared_edt_1d_parabolic( f + last * stride, n - last, stride, anisotropy, ( black_border || last > 0 ),
                               black_border );
  }
}
 ...
 */
template <typename T>
float *_edt3dsq( T *labels, const size_t sx, const size_t sy, const size_t sz, const float wx, const float wy,
                 const float wz, const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
  ...
  {
    pool.enqueue(
        [ labels, sy, z, sx, sxy, wx, workspace, black_border ]()
        {
          for ( size_t y = 0; y < sy; y++ )
          {
            squared_edt_1d_multi_seg<T>( ( labels + sx * y + sxy * z ), ( workspace + sx * y + sxy * z ), sx, 1, wx,
                                         black_border );
          }
        } );
  }
  ...
  {
    pool.enqueue(
        [ labels, sxy, z, workspace, sx, sy, wy, black_border ]()
        {
          for ( size_t x = 0; x < sx; x++ )
          {
            squared_edt_1d_parabolic_multi_seg<T>( ( labels + x + sxy * z ), ( workspace + x + sxy * z ), sy, sx, wy,
                                                   black_border );
          }
        } );
  }
  ...
  {
    pool.enqueue(
        [ labels, sx, y, workspace, sz, sxy, wz, black_border ]()
        {
          for ( size_t x = 0; x < sx; x++ )
          {
            squared_edt_1d_parabolic_multi_seg<T>( ( labels + x + sx * y ), ( workspace + x + sx * y ), sz, sxy, wz,
                                                   black_border );
          }
        } );
  }
...
template <typename T>
float *_binary_edt3dsq( T *binaryimg, const size_t sx, const size_t sy, const size_t sz, const float wx, const float wy,
                        const float wz, const bool black_border = false, const int parallel = 1,
                        float *workspace = NULL )
{
  ...
  {
    pool.enqueue(
        [ binaryimg, sy, sx, sxy, z, workspace, wx, black_border ]()
        {
          for ( size_t y = 0; y < sy; y++ )
          {
            squared_edt_1d_multi_seg<T>( ( binaryimg + sx * y + sxy * z ), ( workspace + sx * y + sxy * z ), sx, 1, wx,
                                         black_border );
          }
        } );
  }
  ...
  {
    pool.enqueue(
        [ sx, sy, sxy, z, workspace, wy, black_border ]()
        {
          for ( size_t x = 0; x < sx; x++ )
          {
            size_t offset = x + sxy * z;
            size_t y;
            for ( y = 0; y < sy; y++ )
            {
            ...
            _squared_edt_1d_parabolic( ( workspace + offset + sx * y ), sy - y, sx, wy, black_border || ( y > 0 ),
                                       black_border );
          }
        } );
  }
  ...
  {
    pool.enqueue(
        [ y, sx, sz, sxy, workspace, wz, black_border ]()
        {
          for ( size_t x = 0; x < sx; x++ )
          {
            size_t offset = x + sx * y;
            size_t z      = 0;
            for ( z = 0; z < sz; z++ )
            {
            ...
            _squared_edt_1d_parabolic( ( workspace + offset + sxy * z ), sz - z, sxy, wz, black_border || ( z > 0 ),
                                       black_border );
          }
        } );
  }
...
template <typename T>
float *_edt3dsq( bool *binaryimg, const size_t sx, const size_t sy, const size_t sz, const float wx, const float wy,
                 const float wz, const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
...
// euclidean distance.
template <typename T>
float *_edt3d( T *input, const size_t sx, const size_t sy, const size_t sz, const float wx, const float wy,
               const float wz, const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt3d( T *input, const size_t sx, const size_t sy, const size_t sz, const float wx, const float wy,
                      const float wz, const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
  float *transform = _binary_edt3dsq<T>( input, sx, sy, sz, wx, wy, wz, black_border, parallel, workspace );
...
template <typename T>
float *_edt2dsq( T *input, const size_t sx, const size_t sy, const float wx, const float wy,
                 const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
  ...
  for ( size_t y = 0; y < sy; y++ )
  {
    squared_edt_1d_multi_seg<T>( ( input + sx * y ), ( workspace + sx * y ), sx, 1, wx, black_border );
  }
  ...
  {
    pool.enqueue(
        [ input, x, workspace, sy, sx, wy, black_border ]()
        { squared_edt_1d_parabolic_multi_seg<T>( ( input + x ), ( workspace + x ), sy, sx, wy, black_border ); } );
  }
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt2dsq( T *binaryimg, const size_t sx, const size_t sy, const float wx, const float wy,
                        const bool black_border = false, const int parallel = 1, float *workspace = NULL )
{
  const size_t voxels = sx * sy;
  size_t x, y;
  ...
  for ( y = 0; y < sy; y++ )
  {
    squared_edt_1d_multi_seg<T>( ( binaryimg + sx * y ), ( workspace + sx * y ), sx, 1, wx, black_border );
  }
#ifdef POOL
  ThreadPool pool( parallel );
#endif
...
#ifdef POOL
    pool.enqueue(
        [ workspace, x, sx, sy, wy, black_border ]()
#endif
        {
          size_t y = 0;
          for ( y = 0; y < sy; y++ )
          ...
          _squared_edt_1d_parabolic( ( workspace + x + y * sx ), sy - y, sx, wy, black_border || ( y > 0 ),
                                     black_border );
        }
#ifdef POOL
    );
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt2d( T *binaryimg, const size_t sx, const size_t sy, const float wx, const float wy,
                      const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  float *transform = _binary_edt2dsq( binaryimg, sx, sy, wx, wy, black_border, parallel, output );

  for ( size_t i = 0; i < sx * sy; i++ ) transform[ i ] = std::sqrt( transform[ i ] );
...
// 2D version of _edt3dsq
template <typename T>
float *_edt2dsq( bool *binaryimg, const size_t sx, const size_t sy, const float wx, const float wy,
                 const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  return _binary_edt2dsq( binaryimg, sx, sy, wx, wy, black_border, parallel, output );
}

// returns euclidean distance instead of squared distance
template <typename T>
float *_edt2d( T *input, const size_t sx, const size_t sy, const float wx, const float wy,
               const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  float *transform = _edt2dsq<T>( input, sx, sy, wx, wy, black_border, parallel, output );
...
template <typename T>
float *edt( T *labels, const int sx, const float wx, const bool black_border = false )
{
...
template <typename T>
float *edt( T *labels, const int sx, const int sy, const float wx, const float wy, const bool black_border = false,
            const int parallel = 1, float *output = NULL )
{
  return pyedt::_edt2d( labels, sx, sy, wx, wy, black_border, parallel, output );
}

template <typename T>
float *edt( T *labels, const int sx, const int sy, const int sz, const float wx, const float wy, const float wz,
            const bool black_border = false, const int parallel = 1, float *output = NULL )
{
...
template <typename T>
float *binary_edt( T *labels, const int sx, const float wx, const bool black_border = false )
{
  return edt::edt( labels, sx, wx, black_border );
}

template <typename T>
float *binary_edt( T *labels, const int sx, const int sy, const float wx, const float wy,
                   const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  return pyedt::_binary_edt2d( labels, sx, sy, wx, wy, black_border, parallel, output );
}

template <typename T>
float *binary_edt( T *labels, const int sx, const int sy, const int sz, const float wx, const float wy, const float wz,
                   const bool black_border = false, const int parallel = 1, float *output = NULL )
{
...
template <typename T>
float *edtsq( T *labels, const int sx, const float wx, const bool black_border = false )
{
  float *d = new float[ sx ]();
...
template <typename T>
float *edtsq( T *labels, const int sx, const int sy, const float wx, const float wy, const bool black_border = false,
              const int parallel = 1, float *output = NULL )
{
...
template <typename T>
float *edtsq( T *labels, const int sx, const int sy, const int sz, const float wx, const float wy, const float wz,
              const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  return pyedt::_edt3dsq( labels, sx, sy, sz, wx, wy, wz, black_border, parallel, output );
}

template <typename T>
float *binary_edtsq( T *labels, const int sx, const float wx, const bool black_border = false, const int parallel = 1 )
{
  return edt::edtsq( labels, sx, wx, black_border );
}

template <typename T>
float *binary_edtsq( T *labels, const int sx, const int sy, const float wx, const float wy,
...
template <typename T>
float *binary_edtsq( T *labels, const int sx, const int sy, const int sz, const float wx, const float wy,
                     const float wz, const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  return pyedt::_binary_edt3dsq( labels, sx, sy, sz, wx, wy, wz, black_border, parallel, output );
}

// signed distance function
template <typename T>
float *sdfsq( T *labels, const size_t sx, const size_t sy, const size_t sz, const float wx = 1, const float wy = 1,
              const float wz = 1, const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  ...
  std::unique_ptr<float[]> complement_dt( edtsq( complement.get(), sx, sy, sz, wx, wy, wz, black_border, parallel ) );
  for ( size_t i = 0; i < voxels; i++ )
  {
...
template <typename T>
float *sdf( T *labels, const size_t sx, const size_t sy, const size_t sz = 1, const float wx = 1, const float wy = 1,
            const float wz = 1, const bool black_border = false, const int parallel = 1, float *output = NULL )
{
  const size_t voxels = sx * sy * sz;
  ...
  std::unique_ptr<float[]> complement_dt( edt( complement.get(), sx, sy, sz, wx, wy, wz, black_border, parallel ) );
  for ( size_t i = 0; i < voxels; i++ )
  {
    output[ i ] -= complement_dt[ i ];
  }
