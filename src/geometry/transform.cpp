...
template void convert_qt_to_tf<double>(const double *qt7, double *tf44,
                                       bool colmajor, bool q_then_t);
template void convert_tf_to_qt<float>(const float *tf44, float *qt7,
                                      bool colmajor, bool q_then_t);
template void convert_tf_to_qt<double>(const double *tf44, double *qt7,
                                       bool colmajor, bool q_then_t);

template void so3_up<float>(const Eigen::Vector3<float> &w,
                            Eigen::Ref<Eigen::Matrix3<float>> so3);

template void so3_up<double>(const Eigen::Vector3<double> &w,
                             Eigen::Ref<Eigen::Matrix3<double>> so3);
...
template <typename REAL>
void convert_qt_to_tf(const REAL *qt7, REAL *tf44, bool colmajor,
                      bool q_then_t) {
...
template <typename REAL>
void convert_tf_to_qt(const REAL *tf44, REAL *qt7, bool colmajor,
                      bool q_then_t) {
  if (colmajor) {
