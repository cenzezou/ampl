...
void test_tf(int argc, char *argv[]) {
  ...
  R_col =
      Eigen::AngleAxisf(0.1, Eigen::Vector3f::Random().normalized()).matrix();
  ...
  tic();
  for (int i = 0; i < trial; i++) {
    ampl::transform_points_soa(
        xyz_X3.col(0).data(), xyz_X3.col(1).data(), xyz_X3.col(2).data(),
        num_xyz, quat.coeffs().data(), t_row.data(), // NOAQ
  ...
  std::cout << (xyz_X3_copy.transpose() - xyz_3X_res).cwiseAbs().maxCoeff()
            << std::endl;
  // return;
  ...
  for (int i = 0; i < trial; i++) {
    ...
    ampl::transform_xyz<float>(quat.coeffs().data(), t_col.data(),
                               xyz_3X.data(), xyz_3X.cols(),
  ...
  std::cout << (xyz_3X_res_quat - xyz_3X_res).cwiseAbs().maxCoeff()
            << std::endl;
  ...
  toc(true);
  std::cout << (xyz_X3_res.transpose() - xyz_3X_res).cwiseAbs().maxCoeff()
            << std::endl;
