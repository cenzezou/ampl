...
def main_arm6():
    """Main function for basic IK."""
    ROBOT_TAG = "elfin_10l"
    path_urdf = f"/home/czhou/Projects/Robotics/pamp_binding/database/robot/elfin10l/urdf.urdf"
    # ROBOT_TAG="hillbot_left"
    # path_urdf= f"/home/czhou/Data/mplib/SoledadAssets-main/hillbot_beta1.0_v2/hillbot_left-link_arm.urdf"
    urdf = yourdfpy.urdf.URDF.load(path_urdf)
    dof = 6
    ...
    mesh.apply_scale(1e-3).apply_transform(
        trimesh.transformations.rotation_matrix(np.pi, [0, 1, 0], [0, 0, 0.5])).apply_translation([0, 0.7, 0.5])
    ...
    timing_handle = server.gui.add_number(
        "1x IK Time (ms)", 0.001, disabled=True)
    ik_target = server.scene.add_transform_controls(
        "/ik_target", scale=0.2,
        position=qt_tool0[4:], wxyz=qt_tool0[[3, 0, 1, 2]]
    )
    ...
    while True:
        ...
        elapsed_time = time.time() - start_time
        timing_handle.value = 0.99 * timing_handle.value + \
