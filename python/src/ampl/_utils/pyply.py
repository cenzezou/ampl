        ...
        try:
            ...
            if isinstance(stream.read(0), str):
                if data.text:
                    ...
                    raise ValueError("can't read binary-format PLY " "from text stream")
            else:
                if data.text:
    ...
    def write(self, stream):
        ...
        try:
            ...
            if binary_stream:
                ...
                if not self.text:
                    raise ValueError("can't write binary-format PLY to " "text stream")
...
class PlyElement(object):
    ...
    def dtype(self, byte_order="="):
        ...
        """
        return _np.dtype(
            [(prop.name, prop.dtype(byte_order)) for prop in self.properties]
        )
    ...
    def _read(self, stream, text, byte_order, mmap, known_list_len={}):
        """
        ...
        if text:
            ...
            list_prop_names = set(
                p.name for p in self.properties if isinstance(p, PlyListProperty)
            )
    ...
    def _read_mmap(self, stream, byte_order, mmap_mode, known_list_len):
        ...
        for p in self.properties:
            if isinstance(p, PlyListProperty):
                ...
                new_dtype.append((p.name, val_dtype, (known_list_len[p.name],)))
        ...
        if max_bytes < num_bytes:
            raise PlyElementParseError(
                "early end-of-file", self, max_bytes // dtype.itemsize
            )
        ...
        for prop in list_len_props:
            ...
            if not len_check.all():
                ...
                raise PlyElementParseError(
                    "unexpected list length", self, row, self.ply_property(prop)
                )
...
class PlyHeaderParseError(PlyParseError):
    ...
    def __repr__(self):
        return "%s(%r, line=%r)" % (self.__class__.__name__, self.message, self.line)
...
class _PlyHeaderParser(object):
    ...
    def consume(self, raw_line):
        ...
        if keyword not in self._allowed:
            self._error("expected one of {%s}" % ", ".join(self._allowed))
    ...
    def parse_property(self, data):
        ...
        if fields[0] == "list":
            ...
            try:
                properties.append(PlyProperty(fields[1], fields[0]))
            except ValueError as e:
                self._error(str(e))
